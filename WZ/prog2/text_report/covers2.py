"""
text_report/covers2.py - last updated 2024-05-10

Use xetex templates for report covers.
...
def run_tex(tex_list, pdf_dir, show_output = False):
    """Convert a list of tex-files to pdf-files.
...
def write_template(
    ...
    tex = REGEX.sub(fsub, template)
    return tex, missing, used
...
# some point ...
        ...
        tex, m, u = write_template(template, fields)
        results.append((sort_name(fields), tex))
...
def save_reports(folder: str, klass, reports: list):
    tex_dir = os.path.join(folder, klass)
    pdf_dir = os.path.join(tex_dir, "pdf")
    #print("§pdf_dir:", pdf_dir)
    os.makedirs(pdf_dir, exist_ok = True)
    tex_list = []
    pdf_list0 = []
    for sname, tex in sorted(reports):
        outpath = os.path.join(tex_dir, sname) + ".tex"
        with open(outpath, 'w') as fh:
            fh.write(tex)
        #print(" -->", outpath)
        tex_list.append(outpath)
        pdf_list0.append(os.path.join(pdf_dir, f"{sname}.pdf"))
    # Clear pdf folder
    for f in os.listdir(pdf_dir):
        os.remove(os.path.join(pdf_dir, f))
    # Generate pdf files
    run_tex(tex_list, pdf_dir, show_output = True)
    # Merge pdf files
    pdf_list = []
    for f in pdf_list0:
        if os.path.exists(f):
