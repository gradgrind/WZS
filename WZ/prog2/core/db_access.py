...
Most of the data is stored as JSON so that the stored data can be easily
mapped to program structures. Generally, only string values are used,
integers are node references ("foreign keys").
"Tables" are still used as a grouping mechanism for the data, but they
are not mirrored as database tables. All the main data is stored in the
table "NODES", which has two (non-index) fields: DB_TABLE (the "table")
and "DATA" (the record).

Foreign keys are not handled by the database backend, but by reference
fields within a node. Such fields have names ending "_id". Their values
are the "id" of the referenced node.

TODO: Should a catalogue of the reference fields be stored in the database?
TODO: The handling of foreign key constraints.
...
#TODO: still needed?
    ...
    def insert(
        self, table: str, fields: list[str], values: list[str | int]
    ) -> int:
...
######-----------------------------------------------------------------
# OLD STUFF: modify or delete
# I probably still need the validation bits, maybe more ...
'''
class DB_FIELD:
