...
DB_Table.add_table(ParallelTags)
...
def grade_report_field(course_data, on):#: COURSE_LINE, on: bool = None) -> bool:
...
def text_report_field(course_data, text):#: COURSE_LINE, text: str = None
#) -> tuple[bool, str, str]:
...
def teachers_print_names(teacher_list: list, value: str):#[db_TableRow], value: str) -> str:
...
def get_pay(teacher: int, course_data, nlessons: int):#: COURSE_LINE, nlessons: int) -> float:
