"""
tables/ods_support.py - last updated 2024-02-20
...
class ODS_Handler:
            ...
            if c["name"] == self.TABLE_ROW:
                ...
                for cc in c["children"]:
                    ...
                    if ccn == self.COVERED_TABLE_CELL:
                        ...
                        used = True
                    try:
                        ln = int(ccattrs[self.REPEAT_COL])
    ...
    @classmethod
    def add_rows(
        cls, elements: list[dict], row: int, n: int
    ) -> list[dict]:
...
def ODS_reader(filepath: str) -> list[list[str]]:
    ...
    def read_only(element: dict) -> bool:
        ...
        Normally the text elements are read, also if the cell is not
        "officially" a text cell, but for dates the actual value must
        be read.
        """
        row = []
        rows.append(row)
        for c in element["children"]:
            try:
                row.append(c["attributes"]["office:date-value"])
            except KeyError:
                row.append(ODS_Handler.cell_text(c))
        #print(" --", rows[-1])
        return False
